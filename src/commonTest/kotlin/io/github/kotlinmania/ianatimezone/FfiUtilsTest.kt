...
class FfiUtilsTest {
    @Test
    fun testAndroidTimezonePropertyNameIsValidCstr() {
    ...
    @Test
    fun testTznameBufferFitsAllIanaNames() {
        val buf = FfiUtils.Buffer.tznameBuf()
        val maxLen = buf.size
        val failedTzNames = mutableListOf<String>()

        for (tz in KNOWN_TIMEZONE_NAMES) {
            if (tz.length >= maxLen) {
                failedTzNames.add(tz)
            }
        }

        assertTrue(
            failedTzNames.isEmpty(),
            "One or more timezone names exceed the buffer length of $maxLen",
        )
    }
    ...
    companion object {
        private val KNOWN_TIMEZONE_NAMES = arrayOf(
            "Africa/Abidjan",
            "Africa/Accra",
            "Africa/Addis_Ababa",
            "Africa/Algiers",
