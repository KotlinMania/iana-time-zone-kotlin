# Claude Code Project Instructions — iana-time-zone-kotlin

## Project Overview

This is **iana-time-zone-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`iana-time-zone`](https://crates.io/crates/iana-time-zone).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/iana-time-zone/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/iana-time-zone>
